...
def print_hi(name):
    ...
    class LF_DNN(nn.Module):
        def __init__(self):
            ...
            self.post_text_prob, self.post_audio_prob, self.post_video_prob, self.post_fusion_prob = (0.5,0.5,0.5,0.5)
            ...
            self.post_text_layer_3 = nn.Linear(self.post_text_dim, 2)
        ...
        while True:
            ...
            with tqdm(train_dataLoader) as td:
                for batch_data in td:
                    ...
                    optimizer.zero_grad()
                    # forward
                    outputs,representations = model2(text)
                    ...
                    y_pred.append(outputs.argmax(1).cpu())
                    y_true.append(labels.squeeze().long().cpu())
                    y_representation.append(representations.cpu())

            pred, true = torch.cat(y_pred), torch.cat(y_true)

            representation = torch.cat(y_representation)
            ...
            if val_acc > best_acc:
                ...
                # save the representations of internal true and representation
                repre_numpy = representation.cpu().detach().numpy()
                true_numpy = true.cpu().detach().numpy()
                res1 = {
                    'repre':repre_numpy,
                    'label':true_numpy
                }

                path =  work_dir + 'representation/text'
