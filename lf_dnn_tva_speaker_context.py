...
def print_hi(name):
    ...
    model_name = 'lf_dnn'
    RESULT_FILE = work_dir + "output/tva_speaker_context/{}.json"
    ...
    class LF_DNN(nn.Module):
        def __init__(self, speakers_num):
            ...
            self.text_hidden, self.context_text_hidden, self.video_hidden, self.audio_hidden, self.speaker_hidden = 32, 32, 128, 16, 4
            # self.text_out = 32
            ...
            self.post_fusion_layer_1 = nn.Linear(self.text_hidden + self.video_hidden + self.audio_hidden
                                                 + self.speakers_num + self.context_text_hidden, self.post_fusion_dim)
