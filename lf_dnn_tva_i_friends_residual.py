...
def print_hi(name):
    ...
    RESULT_FILE = work_dir + "output/independent/tva/residual/{}.json"
    ...
    class LF_DNN(nn.Module):
        def __init__(self):
            ...
            # self.post_fusion_layer_1 = nn.Linear(self.text_hidden + self.audio_hidden + self.video_hidden, self.post_fusion_dim1 )
            self.post_fusion_layer_1 = nn.Linear(self.text_hidden + self.audio_hidden + self.video_hidden+self.text_in  +self.video_in + self.audio_in, self.post_fusion_dim1 )
            ...
            self.post_fusion_layer_6 = nn.Linear(self.post_fusion_dim1 + self.text_in +self.video_in + self.audio_in, self.post_fusion_dim1)

            self.post_fusion_layer_7 = nn.Linear(self.post_fusion_dim1, self.post_fusion_dim1)
            self.post_fusion_layer_8 = nn.Linear(self.post_fusion_dim1, self.post_fusion_dim)
        ...
        def forward(self, text_x, video_x, audio_x):
            ...
            # 128+32+16 = 176
            fusion_h = torch.cat([text_h, video_h ,audio_h, text_x, video_x, audio_x], dim=-1)
            # fusion_h = torch.cat([text_h, video_h ,audio_h], dim=-1)
            ...
            # x = torch.cat([x,text_h, video_h, audio_h], dim=-1 )
    ...
    def do_train(model2, train_dataLoader, val_dataLoader):
        ...
        while True:
            ...
            print('train split confusion_matrix(y_true, y_pred)')
