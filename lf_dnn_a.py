...
def print_hi(name):
    ...
    class LF_DNN(nn.Module):
        def __init__(self):
            ...
            self.post_audio_layer_3 = nn.Linear(self.post_audio_dim, 2)


        def forward(self, text_x):
            audio_h = self.audio_subnet(text_x)
            # audio
    ...
    for fold, (train_index, test_index) in enumerate(split_indices):
        ...
        model4.to(device)
        # do test
        val_acc, y_pred, y_true = do_test(model4, test_dataLoader, mode="TEST")
        del model4
        print('Test: ', val_acc)
        # print(pred,true)
