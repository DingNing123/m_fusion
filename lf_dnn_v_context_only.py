...
def print_hi(name):
    ...
    model_name = 'lf_dnn'
    RESULT_FILE = work_dir + "output/v_context_only/{}.json"
    ...
    class LF_DNN(nn.Module):
        def __init__(self, speakers_num):
            ...
            self.text_hidden, self.context_text_hidden, self.video_hidden, self.audio_hidden, self.speaker_hidden = 128, 32, 128, 16, 8
            # self.text_out = 32
            self.post_fusion_dim = 128
            self.post_fusion_dim1 = 32
            ...
            self.post_fusion_layer_1 = nn.Linear( self.video_hidden + self.context_text_hidden , self.post_fusion_dim)
            self.post_fusion_layer_2 = nn.Linear(self.post_fusion_dim, self.post_fusion_dim)
            self.post_fusion_layer_4 = nn.Linear(self.post_fusion_dim + self.context_text_hidden, self.post_fusion_dim1)
            self.post_fusion_layer_5 = nn.Linear(self.post_fusion_dim1, self.post_fusion_dim1)
            self.post_fusion_layer_6 = nn.Linear(self.post_fusion_dim1, self.post_fusion_dim1)



            self.post_fusion_layer_3 = nn.Linear(self.post_fusion_dim1, 2)
        ...
        def forward(self, text_x, video_x, audio_x, speaker_x, context_text_x):
            ...
            # 128+32+16 = 176
            fusion_h = torch.cat([video_h , context_text_h], dim=-1)
            ...
            x = torch.cat([x,context_text_h],dim=-1)
