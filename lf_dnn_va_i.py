...
def print_hi(name):
    ...
    RESULT_FILE = work_dir + "output/independent/va/no_residual/{}.json"
    ...
    class LF_DNN(nn.Module):
        def __init__(self):
            ...
            self.post_fusion_layer_1 = nn.Linear(self.audio_hidden + self.video_hidden,2 )
        ...
        def forward(self, text_x, video_x, audio_x):
            ...
            # 128+32+16 = 176
            fusion_h = torch.cat([ video_h ,audio_h ], dim=-1)
