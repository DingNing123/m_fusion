...
def print_hi(name):
    ...
    model_name = 'lf_dnn'
    RESULT_FILE = work_dir + "output/tva_i_oursplit_context_only/{}.json"
    ...
    class LF_DNN(nn.Module):
        def __init__(self, speakers_num):
            ...
            self.post_fusion_layer_1 = nn.Linear(self.text_hidden + self.audio_hidden + self.video_hidden + self.text_in
                                                 + self.video_in + self.audio_in + self.context_text_in, self.post_fusion_dim1)
            ...
            self.post_fusion_layer_6 = nn.Linear(self.post_fusion_dim1 + self.text_in + self.video_in + self.audio_in + self.context_text_in,
            ...
            self.post_fusion_layer_9 = nn.Linear(
                self.post_fusion_dim2 + self.context_text_hidden,
        ...
        def forward(self, text_x, video_x, audio_x, speaker_x, context_text_x):
            ...
            # 128+32+16 = 176
            fusion_h = torch.cat([video_h, text_h, audio_h,text_x, video_x, audio_x,context_text_x], dim=-1)
            ...
            x = torch.cat([x, text_x, video_x, audio_x,context_text_x], dim=-1)
